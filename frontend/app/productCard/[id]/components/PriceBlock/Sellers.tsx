...
export const Sellers = ({ price = 0, store, buyLink, data }: any) => {
  ...
  useEffect(() => {
    ...
    const isInConfigurator = userCompilation[data.componentType]?.find(
      (component: any) => {
        return component.id === data.id && component.offers[0].store.name === store.name;
      }
    );
    if (isInConfigurator) {
  ...
  const handleAddToConfigurationClick = () => {
    ...
    if (isAddedToConfigurator === true) {
      dispatch(removeComponent<any>({component: data, store: store.name}));
      setIsAddedToCounfigurator(false);
      return;
    }
    dispatch(addComponent<any>({ data, store: store.name }));
    setIsAddedToCounfigurator(true);
  };
  return (
    <div className={styles.sellersContainer}>
      <p className={styles.sellersShop}>{store.name}</p>
      <p className={styles.sellersPrice}>{price.toLocaleString()}₽ </p>
      <div className={styles.buttonsContainer}>
        <button
          onClick={handleAddToConfigurationClick}
          className={styles.sellersConfiguratorButton}
        >
        ...
        <a href={buyLink} target="_blank" className={styles.sellersBuyButton}>
          <p>Купить</p>
        </a>
      </div>
    </div>
