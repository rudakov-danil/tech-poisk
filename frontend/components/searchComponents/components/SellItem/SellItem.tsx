...
export function SellItem({ elem, e }: any) {
  const [isAddedToConfigurator, setIsAddedToCounfigurator] = useState(false);
  const userCompilationList = useAppSelector(
    (state) => state.userComponentsListReducer
  );
  // useEffect(() => {
  //   if (
  //     userCompilationList[elem.componentType]?.find(
  //       (component: any) =>
  //         component.id === elem.id &&
  //         component.offers[0].store.name === e.store.name
  //     )
  //   ) {
  ...
  //   }
  // }, [userCompilationList[elem.componentType]]);

  const dispatch = useAppDispatch();
  const addToConfigurator = () => {
    if (isAddedToConfigurator === true) {
      dispatch(removeComponent<any>({component: elem, store: e.store.name}));
      setIsAddedToCounfigurator(false);
      return;
    }
    dispatch(addComponent<any>({ data: elem, store: e.store.name }));
    setIsAddedToCounfigurator(true);
  };
  ...
  useEffect(() => {
    ...
    if(userCompilation === null) return
    const isInConfigurator = userCompilation[elem.componentType]?.find(
      (component: any) => {
        return component.id === elem.id && component.offers[0].store.name === e.store.name;
      }
    );
    if (isInConfigurator) {
      setIsAddedToCounfigurator(true);
  ...
  }, [isAddedToConfigurator, userCompilationList]);

  // const addToConfiguratorDiv = (
  //   <div
  //     className={`${styles["box"]}bg-[#D9D9D9] rounded-[5px] shadow-xl py-[5px] px-[12px]`}
  //   >
  //     <p className="text-[12px]">Добавить в конфигуратор</p>
  //   </div>
  // );
  return (
    <div
      onClick={() => addToConfigurator()}
      className="flex cursor-pointer flex-col gap-[12px] mr-[25px]"
      key={e.id}
    >
      <div className="flex gap-[8px] items-center">
        <div
          className={`${styles["box-in-hover"]} p-[3px] rounded-[50%] hover:bg-[#FF5252] border-[#FF5252] border-[1px] flex items-center w-[30px] h-[30px]`}
        >
          {/* {addToConfiguratorDiv} */}
          {isAddedToConfigurator ? (
            <svg
              width="24"
              height="24"
              viewBox="0 0 16 12"
        ...
        <p className="text-[18px] text-[black] hover:underline hover:text-[#FF5252]">
          {e.store.name} - {e.price.toLocaleString()}₽
        </p>
      </div>
    </div>
  );
